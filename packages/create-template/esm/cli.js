...
function _cli() {
  _cli = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
    var pkgJSON, opts, templateConfig;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          pkgJSON = readJSONSync(path.join(__dirname, '../package.json'));
          program.version(pkgJSON.version, '-v, --version', 'Output the current version.').option('-c, --template-config <template-config>', 'Template config path.').option('-t, --app-type <app-type>', 'Template app type.').option('-n, --app-name <app-name>', 'Template app name.').option('-f, --force', 'Override when dest exists file.');
          ...
          if (opts.templateConfig) {
            if (!isPathExistsSync(opts.templateConfig)) {
              logger.printErrorAndExit("".concat(opts.templateConfig, " is not exist."));
            }
            templateConfig = require(opts.templateConfig);
          }
          return _context.abrupt("return", new CreateTemplate(_objectSpread(_objectSpread({}, opts), {}, {
            args: opts,
